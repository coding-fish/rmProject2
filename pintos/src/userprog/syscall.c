...
// // 中断处理函数 provide pointer check and ensure synchronization.
...
// static void (*syscall_handlers[SYS_CALL_NUM])(struct intr_frame *); //中断向量表
// void
// syscall_init (void) 
// {
//   intr_register_int (0x30, 3, INTR_ON, syscall_handler, "syscall"); //由syscall_handler进行中断处理
//   syscall_handlers[SYS_HALT] = &sys_halt;//依次将各中断处理函数注册到向量表中
...
// {
//   //printf ("system call!\n");
//   if (!is_valid_pointer(f->esp, 4))               // 检查栈指针是否有效
//   {
//     exit(-1);
//     return;
//   }
//   int syscall_num = * (int*)f->esp;               // 取出中断号
//   if (syscall_num<=0 || syscall_num>=SYS_CALL_NUM)// 检查中断号是否有效
//   {
//     exit(-1);
//   }
//   syscall_handlers[syscall_num](f);               // 执行中断处理函数
//   // thread_exit ();
// }
